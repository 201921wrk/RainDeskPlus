...
// RainDeskPlus adapter: ensure bare `Skin*` / `Meter*` inside this header
// consistently refers to raindock:: types (which live in `namespace raindock`
// per M4/M5 skeleton).  Using-declarations at global scope mirror what the
// upstream Library/StdAfx.h shim does via `using namespace raindock;`, but
// cover the case when Mouse.h is included from a non-upstream TU (e.g. our
// local Skin.cpp) WITHOUT that umbrella directive active.
namespace raindock { class Skin; class Meter; class ConfigParser; }
using raindock::Skin;
